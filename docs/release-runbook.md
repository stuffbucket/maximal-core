...
```sh
bun run release:manual    # preflight; bumpp prompts for the version; commits, tags, pushes
```

`release:manual` is `release:preflight && bumpp && bun publish --access public`.
The preflight is first **because everything after it is irreversible**: `bumpp`
commits, tags and pushes before `bun publish` is ever reached, and a registry
publish cannot be taken back at all. It asserts one thing — that the Bun about
to run is the one in `.bun-version`:
...
> surprises people.** Bun runs lifecycle scripts through a shell whose PATH has
> neither `node_modules/.bin` nor Bun's own bindir on it, so a bare `bun` inside
> a script re-resolves from *your* PATH. Invoking the pinned binary directly
> still produced the unpinned bundle:
>
> ```
> $ /path/to/1.3.11/bin/bun pm pack     # tarball dist/main.js → ffdee378… (1.3.14)
> ```
>
> That is why `prepack` is [`scripts/ops/prepack.ts`](../scripts/ops/prepack.ts)
...
> that was checked is the binary that bundles. It refuses with a non-zero exit
> rather than downloading a Bun for you, and it refuses **before** writing
> anything into `dist/`. The same script backs the preflight (`--check` asserts
> without building), so `bun publish` and `bun pm pack` are guarded even when run
> by hand, outside `release:manual`.
...
> **Regenerate `dist/main.js` in the release commit.** `bun build` inlines
> `package.json` — `BUILD_VERSION` in `src/lib/update/build-info.ts` falls back
> to `packageJson.version` — so bumping the version alone makes the committed
> bundle stale and turns `bindings:check` red on `main`. It is genuine drift,
> not a false positive: that bundle is the `bin` a git-dependency install runs,
> and it would print and report the previous version. Measured, same Bun 1.3.11,
> one bumped version: `85697a48…` at `0.3.2` vs `2e541596…` at `0.3.3`. Every
> release so far has been regenerated by hand (`git show v0.3.2:dist/main.js`
> contains `0.3.2`, not `0.3.1`); this line is that step, written down.
> **`bumpp` does not do it for you** — after `bun run release:manual` you must
> still regenerate and amend, on the pinned Bun.
