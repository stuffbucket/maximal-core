...
| Manual deep | `check:deep` script | On demand, pre-PR | minutes | check:fast + casts:check + tests + dead-code finder + dependency-layer rules + build + downstream typecheck + committed-artifact freshness |
