...
/**
 * Cancel an in-flight device-code flow WITHOUT signing out. Aborts the poller
 * and returns to wherever the flow started from: the prior signed-in account if
 * there was one (so "sign in as a different account" → Cancel keeps you on your
 * current account), otherwise signed-out. The existing token/registry were never
 * touched by starting the flow, so the restored session is fully live. No-op
 * (returns the current status) when there is no active flow.
 */
